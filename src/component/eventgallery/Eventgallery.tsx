...
export default function Gallery() {
  ...
  return (
    <div className={`row ${styles.row4} `}>
      <h2>EVENT GALLERY</h2>
      <div className={`col-lg-6 col-6 ${styles.btn} ${content ? styles.clr : ""}`} onClick={handler} >
      <FontAwesomeIcon icon={faImage}  className={`${styles.mic}  ${content ? styles.clr2 : styles.clr3}`} />
      </div>
      <div className={`col-lg-6 col-6 ${styles.btn}  ${content ? "" :styles.clr }`} onClick={handler}>
      <FontAwesomeIcon icon={faVideo} className={`${styles.mic} ${content ? styles.cl3: styles.clr2}`}/>
      </div>
      {
        content ? (
            <div className="row">
